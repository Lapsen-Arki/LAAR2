...
class ChatRobotService {
  ...
  public setOnMessageReceived(
    callback: (response?: Message | Message[]) => void
  ): void {
  ...
  private async getWeatherForecast(
    ...
    try {
      ...
      let todayForecast = `Tässä on ${
        type === "today" ? "tämän päivän" : "6 päivän"
      } sää kaupungista ${city}:<br /><br />`;
      ...
      if (type === "today") {
        ...
        for (const day of Object.keys(organizedForecasts)) {
          ...
          const forecasts = organizedForecasts[day]
            .map(
              (forecast) =>
                `${forecast.time}, ${forecast.description}, lämpötila: ${forecast.temperature}°C`
            )
            .join("<br />");
  ...
  private async checkCityExistence(
    city: string,
    apiKey: string
  ): Promise<boolean> {
  ...
  public async addUserMessageAndGenerateResponse(
    ...
    if (userMessage.trim() !== "") {
      ...
      if (response && this.onMessageReceivedCallback) {
        const resolvedResponse = (await Promise.resolve(response)) as
          | Message
          | Message[];
        // If response is a single Message, wrap it in an array
        const messagesToSend = Array.isArray(resolvedResponse)
          ? resolvedResponse
          : [resolvedResponse];
  ...
  private handleUserInput(input: string): string | undefined {
    const [command, topic] = input.split(" ");

    if (
      command === "help" &&
      topic &&
      Object.prototype.hasOwnProperty.call(robotAnswers, topic)
    ) {
      return robotAnswers[topic];
    } else if (Object.prototype.hasOwnProperty.call(robotAnswers, input)) {
      return robotAnswers[input];
    } else {
      return undefined; // Return undefined for unrecognized input
    }
  }
  ...
  private async generateResponse(
    ...
    // Check if the user message matches any additional messages
    if (
      trimmedUserMessage.includes("mitä botti osaa tehdä") ||
      trimmedUserMessage.includes("mitä osaat tehdä") ||
      trimmedUserMessage.includes("miten voit auttaa minua")
    ) {
      const additionalMessage: Message = {
        id: "additionalMessage",
        senderId: "LAAR Chattirobotti",
        receiverId: "User",
        message: "Mitä botti osaa tehdä?",
        text: `
          <p>Voin auttaa sinua monilla eri tavoilla. Tässä muutamia esimerkkejä:</p>
          <ul>
              <li>Voit kysyä minulta tietoa esimerkiksi säästä, liikunnasta, ruokailusta ja muista aiheista.</li>
              <li>Voin antaa sinulle neuvoja ja vinkkejä erilaisiin arkipäivän haasteisiin liittyen esimerkiksi lasten kanssa.</li>
              <li>Voin auttaa sinua löytämään tietoa LAAR-applikaatiosta ja sen käytöstä.</li>
              <li>Voit myös kertoa minulle, jos kohtaat ongelmia palvelun käytössä, ja yritän auttaa sinua ratkaisemaan ne.</li>
          </ul>
          <p>Kirjoita rohkeasti kysymyksesi, ja teen parhaani auttaakseni sinua!</p>
        `,
        isUser: false,
        timestamp: new Date(),
      };
    ...
    // Inside your generateResponse method or wherever you handle user input
    const bugKeywords = ["bugi", "bug", "virhe", "error"]; // Add more keywords as needed

    for (const keyword of bugKeywords) {
      if (trimmedUserMessage.includes(keyword)) {
        const bugReportMessage: Message = {
          id: "bugReportMessage",
          senderId: "LAAR Chattirobotti",
          receiverId: "User",
          message: "Palvelussa on bugi, minne voin ilmoittaa siitä?",
          text: robotAnswers.bugi,
          isUser: false,
          timestamp: new Date(),
        };
    ...
    // New part - handle user input using handleUserInput function
    const userInputResponse = this.handleUserInput(trimmedUserMessage);
    if (userInputResponse) {
      const response: Message = {
        ...
        text: `LAAR Chattirobotti: ${userInputResponse}`,
        timestamp: new Date(),
        isUser: false,
      };
      return response;
    ...
    const matches = stringSimilarity.findBestMatch(
      trimmedUserMessage.toLowerCase(),
    ...
    if (/sää|weather/i.test(trimmedUserMessage)) {
      try {
        ...
        const typeMatch = trimmedUserMessage.match(
          /(tänään|today|viikon|week)/i
        );
    ...
    const defaultResponseMessage =
      "En ymmärtänyt kysymystäsi. Voisitko kertoa lisää?";
