...
const updateSessionStorageWithProfile = (profile: ChildProfile) => {
    ...
    const storedProfiles = storedProfilesJson ? JSON.parse(storedProfilesJson) as ChildProfile[] : [];
...
export const getChildProfileById = async (id: string, idToken: string | null) => {
    try {
        ...
        const response = await axios.get<ChildProfile>(`${API_BASE_URL}/profile/${id}`, config);
