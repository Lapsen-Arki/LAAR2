...
const checkAuth = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const idToken = req.headers.authorization; // Oletetaan, että idToken on lähetetty pyynnön otsakkeena

    if (!idToken) {
      res.status(401).json({ error: 'Kirjaudu sisään käyttääksesi tätä toimintoa' });
      return;
    }
